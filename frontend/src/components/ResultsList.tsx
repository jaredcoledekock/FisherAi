...
import { Area, PlanWindow } from "../api/types";
...
function ResultsList({ results, area, userId }: Props) {
    ...
    <div className="results">
      ...
      <div className="grid">
        {results.slice(0, 8).map((row) => {
          const key = `${row.date}-${row.window_id}`;
          const chartData =
            row.day_windows && row.day_windows.length
              ? row.day_windows.map((w) => ({
                  name: w.window_id,
                  wind: w.wind_speed,
                  swell: w.swell_height,
                  temp: w.sea_temp,
                }))
              : [
                  {
          ...
          return (
            <div key={key} className="result-card">
              <div className="result-meta">
                <strong>
                  {format(parseISO(row.date), "EEE d MMM")} · {row.window}
                </strong>
              ...
              <div style={{ height: 120 }}>
                <ResponsiveContainer>
                  ...
                  >
                    ...
                    />
                    <Bar dataKey="wind" fill="var(--accent)" radius={[6, 6, 0, 0]} />
                    <Bar dataKey="swell" fill="var(--accent-2)" radius={[6, 6, 0, 0]} />
                    <Line type="monotone" dataKey="temp" stroke="#b7c4e1" strokeWidth={2} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
